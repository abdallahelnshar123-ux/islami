import 'package:flutter/material.dart';

class AppColors {
  static const Color transparentColor = Colors.transparent;
  static const Color primaryColor = Color(0xffE2BE7F);
  static const Color blackColor = Color(0xff202020);
  static const Color black70Color = Color.fromRGBO(32, 32, 32, .7);
  static const Color black90Color = Color.fromRGBO(32, 32, 32, .9);
  static const Color blackBgColor = Color(0x3C202020);
  static const Color whiteColor = Color(0xffffffff);
  static const Color grayColor = Color(0xff707070);
  static const Color brownColor = Color(0xff856B3F);
}
